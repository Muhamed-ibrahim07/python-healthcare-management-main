...
def appointments():
    ...
    options = ["Book Appointment", "Update Appointment", "View Appointments"]
    choice = st.sidebar.selectbox("Select an action:", options)
...
def patients():
    st.header('PATIENTS')
    option_list = ['', 'Add patient', 'Update patient', 'Delete patient','Update Medical Information','Appoinments']
    option = st.sidebar.selectbox('Select function', option_list)
    p = Patient()


    if (option == option_list[1] or option == option_list[2] or option == option_list[3] or option == option_list[4] or option == option_list[5]) and verify_edit_mode_password():
        if option == option_list[1]:
            ...
            st.subheader('UPDATE MED INFO')
            prescriptions()
        elif option == option_list[5]:
            st.subheader('APPOINMENTS')
            appointments()
    # elif option == option_list[4]:
...
def doctors():
    st.header('DOCTORS')
    option_list = ['', 'Add doctor', 'Update doctor', 'Delete doctor']
    option = st.sidebar.selectbox('Select function', option_list)
    dr = Doctor()
...
def home():
    ...
    option = st.sidebar.selectbox('Select module', ['', 'Patients', 'Doctors'])
    if option == 'Patients':
        patients()
    elif option == 'Doctors':
        doctors()
...
st.title(':red[_MEDIBOLT_](Healthcare information management system)')
st.image('OIP.jpg')
