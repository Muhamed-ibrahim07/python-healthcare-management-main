    ...
    with conn:
        c.execute(
            ...
            CREATE TABLE IF NOT EXISTS appointment_record (
                ...
                FOREIGN KEY (patient_id) REFERENCES patient_record(id) ON UPDATE CASCADE ON DELETE RESTRICT,
                FOREIGN KEY (doctor_id) REFERENCES doctor_record(id) ON UPDATE CASCADE ON DELETE RESTRICT
            )
            """
        )
    with conn:
